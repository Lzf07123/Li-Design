...
1. **模板仓库自包含**：本仓库的规范条款不依赖、不引用、不提及任何外部项目仓库；全部令牌、组件、示例取值与验收标准内联在本仓库，克隆到任意位置即可独立使用（家族实现总览仅作审计记录，不构成规范依赖）。
2. **后续项目必须达到一模一样的效果**：项目必须从本模板**同栈实例化**——样式层统一 Tailwind CSS 4（`@theme` 语义别名 + 与参考实现相同的 utility class），禁止手写 CSS 近似；任何视觉效果必须与参考实现**像素级一致（一模一样）**，验收以截图逐值核对为准（见 §6 清单）。
...
- [ ] 技术栈一致（V1.5.2）：样式层 Tailwind CSS 4 同构（`@theme` 语义别名 + 与参考实现相同的 utility class），无手写 CSS 近似；效果与参考实现像素级一致（截图逐值核对）
- [ ] 浅色模式正文对比度 ≥ 4.5:1
